...
/**
 * Swap all group items with this one element, but only after the images in the
 * new element have loaded.
 */
Group.prototype.swapItemsWithElementWhenLoaded = function(element) {
	var loadedImgCount, totalImgCount, swapIfReady, imgFinished, images, swapFunc, offscreenElement;
	var groupElement = this.controller.get('items_' + this.id);

	swapFunc = function() {
		if (groupElement.childNodes.length > 0) {	// There's only ever one child
			groupElement.replaceChild(element, groupElement.firstChild);
		}
		else {
			groupElement.appendChild(element);
		}

// FIND A WAY TO REMOVE 'Dloading' from Old group if present;		

		// Now remove my offscreen div that was just used to hold the element
		// while it's images were loading.
		document.body.removeChild(offscreenElement);
	};

	swapIfReady = function() {
		if (loadedImgCount === totalImgCount) 
		{
		    Mojo.Log.error("loadedImgCount=" + loadedImgCount+"==" + totalImgCount);
			swapFunc();
		} else {
		    Mojo.Log.error("loadedImgCount=" + loadedImgCount+"!=" + totalImgCount);
		}
	...
	loadedImgCount = 0;
	totalImgCount = 0;
	images = element.getElementsBySelector('img');
	
	for (i = 0; i != images.length; i++) {
	        if (!images[i].src.startsWith('file:')) {
		    totalImgCount += 1;
		}
	}

	images.each(function(img) {
		img.onload  = imgFinished;
		img.onerror = imgFinished;
		img.onabort = imgFinished;		
	});

	offscreenElement = new Element('div');

	document.body.appendChild(offscreenElement);
	offscreenElement.appendChild(element);
	offscreenElement.hide();
...
Group.prototype.updateContent = function(section){
	...
	var dynamicContent, that, groupElement;
    this.section = section;
    ...
    if (section.content && (section.content.length > 0)) {
		...
		dynamicContent = this.createDynamicContent(section.content);

		// let's check if a canvas was added to the page
		that = this;
		this.canvases.each(function(canvas) {
			that.updateCanvas(canvas.canvasId, canvas.canvasData, dynamicContent);
		});

		this.swapItemsWithElementWhenLoaded(dynamicContent);
    }
	else {
		 
		groupElement = this.controller.get('items_' + this.id);
		while (groupElement.hasChildNodes()) {
...
Group.prototype.updateCanvas = function(canvasId, canvasData, groupElement){
    ...
    var div1canvas;
	if (groupElement) {
		images = groupElement.getElementsBySelector('#'+canvasId);
		div1canvas = images[0];
	}
	else {
		div1canvas = document.getElementById(canvasId);
	}
    ...
    if (mediaId) {
		...
		if (url) {
			...
			if (title) {
				...
				params.title = 'Nascar Video Stream';
			}
